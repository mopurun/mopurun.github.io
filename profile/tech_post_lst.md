...
鉛筆配線でビアを作る。導電ペンでもやってみる。電源をボタン電池にする。
鉛筆で抵抗器を作る
マイナ保険証の顔認証画像が出ないというハプニングが起きる→起動画面を拝めた
１０しか見たことなかった…50初めて見る☆
壊れたスイッチの交換作業→成功  
翔泳社さんの、日めくりカレンダー企画「サンプルコード365+1」にNASM（アセンブリ言語）とBrainf**kのサンプルコードを投稿したら掲載されることになりました
Suzuno32RVに挿せる簡単な拡張基板を作ってみました 
オシロスコープでイラストをみる☆
アセンブリ言語でマイコンのLチカ
照度センサーを紙に貼り付けてLEDが暗くなるか実験 。赤外線リモコンの動作チェック
ASCIIアートでまいこん
描いたら導電ペンが断線するペンを見つける  
0603mmサイズのLEDを導電ペンで配線して光らせる実験  
アクセサリー作ってみた☆本物のマイコンを使用したまいこん
導電ペンで交差する配線
電子部品を紙に貼り付けてNOTゲート回路 
秋月電子のテルミンもどきはんだ付け練習キットを組み立ててみた  
紙に太陽電池貼り付けてLEDを光らせる実験
手作りの抵抗器はUSB Type-Cで認識してくれるか→認識されて5Vが給電された
  
2025/11

オリジナルスタンプを作ってみる
ALIENTEKのはんだごてのこて台やマイコンボードを買ってみた
なにもしない基板(I2CtoOLED)を作る。ポストが伸びる
暴れるアナログ時計やデバッグ用の爆速アナログ時計をプログラミングでつくってOLEDに表示
子どもの頃に鉛筆で描いたジグザグ線（抵抗器のシンボル）をテスターで抵抗値を測定した思い出（インプレッション12万）
鉛筆で配線を書いて、アートと組み合わせて本物のLEDを光らせてみた
8Bの鉛筆で配線描いて4.6V加えてみたらLED光った（インプレッション9.3万）
鉛筆配線にスイッチを付ける。LEDを接着する
OLEDディスプレイに画像表示する
AndroidのLinuxターミナルにslをインストールして遊ぶ  
2025/10
...
CR2025を消費したくてCR2032の電池ボックスにCR2025を入れる
Raspberry Pi 5にペンタブレットを繋いでみたらすんなり繋がってビックリ
某マイコンボードのイラストをPowerPointで作ってみた  
